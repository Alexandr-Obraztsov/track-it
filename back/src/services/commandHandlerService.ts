...
export class CommandHandlerService {
    ...
    async handleRegister(bot: TelegramBot, msg: any): Promise<void> {
        ...
        if (!isGroup) {
            bot.sendMessage(chatId, 'Регистрация доступна только в групповых чатах')
            return
        }
        ...
        try {
            ...
            if (existingMember) {
                bot.sendMessage(chatId, `${msg.from!.first_name || msg.from!.username}, вы уже зарегистрированы в этой группе!`)
                return
            }
			...
			bot.sendMessage(chatId, `✅ ${member.firstName || member.username} успешно зарегистрирован в группе!`)
        } catch (error) {
